...
def run_resolution_enhancement(
    ...
    ax[0].imshow(enhance.first_scene)
    ax[0].set_title("Scene view, first frame")
    ax[1].imshow(interp_first[:, :, rgb_idx] / interp_first[:, :, rgb_idx].max())
    ax[1].set_title("Single shot upsampled, first frame")
    ax[2].imshow(interp_highres[:, :, rgb_idx] / interp_highres[:, :, rgb_idx].max())
    ax[2].set_title("Resolution enhanced")
    plt.savefig(
        os.path.join(
            Path(filepath).parent.absolute(), Path(filepath).stem + "-enhanced.png"
        )
    )
    plt.show()
    return interp_highres
